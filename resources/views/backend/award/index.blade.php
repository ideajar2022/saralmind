        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6 text-right">
            <a href="{{route('award.create')}}" class="badge bg-success top-add-btn"><i class="fa fa-plus" title="Add award"></i></a>
             <a href="{{route('award.trash')}}" class="badge bg-primary top-add-btn"><i class="fa fa-eye" title="View Trashed"></i></a>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
                ...
                <table class="table table-bordered">
                  ...
                  @foreach($awards as $key=>$award)
                    <tr id="table-row-{{ $award->id }}">
                        ...
                        <td>
                           <img width="100px" class="table-team-img" src="{{ ($award->image_url) }}">
                        </td>
                        <td>{{ $award->title }}</td>
                        <td>{{ $award->awarded_at }}</td>
                        <td>{{ $award->status }}</td>
                        <td>
                          <a href="{{ route('award.edit',$award->id) }}" class="badge bg-primary"><i class="fas fa-edit"></i></a>
                          <a href="{{ route('award.destroy',$award->id)}}" class="badge bg-danger delete-confirm"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Delete"></i></a>
                        </td>
                    </tr>
                  @endforeach
                  </tbody>
