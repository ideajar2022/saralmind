      ...
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6 text-right">
            <a href="{{route('unit.create')}}" class="badge bg-success top-add-btn"><i class="fa fa-plus" title="Add Unit"></i></a>
            <a href="{{route('unit.trash')}}" class="badge bg-primary top-add-btn"><i class="fa fa-eye" title="View Trashed"></i></a>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
              ...
              <div class="card-body">
                <table class="table table-bordered">
                  ...
                  @foreach($units as $key=>$unit)
                    <tr id="table-row-{{ $unit->id }}">
                        <td>{{ ($units->currentpage()-1) * $units->perpage() + $key + 1 }}</td>
                        <td>
                         <img width="100px" class="table-team-img" src="{{ ($unit->image_url) }}">
                        </td>
                        <td>{{ $unit->name }}</td>
                        ...
                        <td>{{ $unit->status }}</td>
                        <td>
                          <a href="{{ route('unit.edit',$unit->id) }}" class="badge bg-primary"><i class="fas fa-edit"></i></a>
                          <a href="{{ route('unit.destroy',$unit->id)}}" class="badge bg-danger delete-confirm"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Delete"></i></a>
                        </td>
                    </tr>
                  @endforeach
                  </tbody>
...
<script type="text/javascript">
  if($("select[name='program_id']").val() != null){
    $("select[name='program_id']").trigger('change',["{{ request('grade_id') }}","{{ request('subject_id') }}"])
