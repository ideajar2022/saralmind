      ...
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6 text-right">
            <a href="{{route('grade.create')}}" class="badge bg-success top-add-btn"><i class="fa fa-plus" title="Add Grade"></i></a>
             <a href="{{route('grade.trash')}}" class="badge bg-primary top-add-btn"><i class="fa fa-eye" title="View Trashed"></i></a>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
              ...
              <div class="card-body">
                <table class="table table-bordered">
                  ...
                  @foreach($grades as $key=>$grade)
                    <tr id="table-row-{{ $grade->id }}">
                        <td>{{ ($grades->currentpage()-1) * $grades->perpage() + $key + 1 }}</td>
                        <td>{{ $grade->name }}</td>
                        <td>{{ @$grade->program->name }}</td>
                        <td>{{ @$grade->faculty->name }}</td>
                        <td>
                        ...
                        <td>
                          <a href="{{ route('grade.edit',$grade->id) }}" class="badge bg-primary"><i class="fas fa-edit"></i></a>
                          <a href="{{ route('grade.destroy',$grade->id)}}" class="badge bg-danger delete-confirm"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Delete"></i></a>
                        </td>
                    </tr>
                  @endforeach
                  </tbody>
