      ...
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6 text-right">
            <a href="{{route('exercise.import')}}"><button type="submit" class="btn btn-primary btn-sm">Import Exercise</button></a>
            <a href="{{route('exercise.create')}}" class="badge bg-success top-add-btn"><i class="fa fa-plus" title="Add Exercises"></i></a>
            <a href="{{route('exercise.trash')}}" class="badge bg-primary top-add-btn"><i class="fa fa-eye" title="View Trashed"></i></a>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
                ...
                <table class="table table-bordered">
                  ...
                  @foreach($subjectiveQuestions as $key=>$subjectiveQuestion)
                    <tr id="table-row-{{ $subjectiveQuestion->id }}">
                        ...
                        <td>{{ $subjectiveQuestion->question }}</td>
                        <td>{{ @$subjectiveQuestion->note->program->name }}</td>
                        <td>{{ @$subjectiveQuestion->note->faculty->name }}</td>
                        <td>{{ @$subjectiveQuestion->note->grade->name }}</td>
                        <td>{{ @$subjectiveQuestion->note->subject->name }}</td>
                        <td>{{ @$subjectiveQuestion->note->unit->name }}</td>
                        <td>{{ @$subjectiveQuestion->note->lesson->name }}</td>
                        <td>{{ @$subjectiveQuestion->note->title }}</td>
                        <td>{{ $subjectiveQuestion->status }}</td>
                        <td>
                          @can('edit-note-subjective-question')
                          <a href="{{ route('exercise.edit',$subjectiveQuestion->id) }}" class="badge bg-primary"><i class="fas fa-edit"></i></a>
                          @endcan

                          @can('delete-note-subjective-question')
                          <a href="{{ route('exercise.destroy',$subjectiveQuestion->id)}}" class="badge bg-danger delete-confirm"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Delete"></i></a>
                          @endcan
                        </td>
                    </tr>
                  @endforeach
