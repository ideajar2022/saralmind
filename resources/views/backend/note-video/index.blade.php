      ...
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6 text-right">
            <a href="{{route('video.create')}}" class="badge bg-success top-add-btn"><i class="fa fa-plus" title="Add Note Videos"></i></a>
             <a href="{{route('video.trash')}}" class="badge bg-primary top-add-btn"><i class="fa fa-eye" title="View Trashed"></i></a>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table class="table table-bordered">
                  ...
                  <tbody>
                  @foreach($noteVideos as $key=>$noteVideo)
                    <tr id="table-row-{{ $noteVideo->id }}">
                        <td>
                          {{ ($noteVideos->currentpage()-1) * $noteVideos->perpage() + $key + 1 }}</td>
                        <td>
                          <a href="{{ $noteVideo->url }}" target="_blank">{{ $noteVideo->title }}</a>
                        </td>
                       
                        <td>{{ @$noteVideo->note->program->name }}</td>
                        <td>{{ @$noteVideo->note->faculty->name }}</td>
                        <td>{{ @$noteVideo->note->grade->name }}</td>
                        <td>{{ @$noteVideo->note->subject->name }}</td>
                        <td>{{ @$noteVideo->note->unit->name }}</td>
                        <td>{{ @$noteVideo->note->lesson->name }}</td>
                        <td>{{ @$noteVideo->note->title }}</td>
                        <td>{{ $noteVideo->status }}</td>
                        <td>
                          @can('edit-note-video')
                          <a href="{{ route('video.edit',$noteVideo->id) }}" class="badge bg-primary"><i class="fas fa-edit"></i></a>
                          @endcan

                          @can('delete-note-video')
                          <a href="{{ route('video.destroy',$noteVideo->id)}}" class="badge bg-danger delete-confirm"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Delete"></i></a>
                          @endcan
                        </td>
                    </tr>
                  @endforeach
